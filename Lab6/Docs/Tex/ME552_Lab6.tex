...
We use a 1st order low pass filter applied to the angle, $\theta$ output by the the pair of accelerometer and a 1st order high pass filter applied to the rate, $\dot{\theta}$ output by the rate gyro.  Both filters have a cut off frequency of \xxx{FREQUENCY $\approx$ 1 Hz} \xxx{describe tuning}  A 1st order filter was selected to limit the phase lag at 1 Hz to 45 degrees.  Higher order filters woudl have introduced a greater amount of lag.   
