...
\xxx{The op-amp is set up as a voltage follower circuit. We do so in order to prevent any down-stream set-up from drawing excessive power from the sensor setup. In this particular case, I don't think we need the buffer op-amp because our ADC is supposed to have high input impedance, thereby preventing any load effects to be seen across the sensor. - Shiva}
\subsection*{d.}
\subsection*{e.}
\subsection*{f.}
