...
\subsection*{b.}
\xxx{We may want to reexamine the pull in frequency using the function generator rather than our own system, its likely the reason for the inconsistent behavior.  We can just use exactly the same method as before to figure it out, just run the vi and then start the function generator}

Pull-in frequencies were measured with the driver in full step mode by starting from rest and using our Lab View system to send a pulse train at a specific frequency with exactly 20 pulses.  We measured the final angle reported by the encoder and computed an equivalent number of steps.  Comparing these two numbers would then tell us if the stepper motor had skipped steps.  However we found that performance was inconsistent.  For a wide range of frequencies in both the loaded and unloaded cases, the system would match the number of pulses within the quantization error introduced by the encoder, but in a few cases the motor would be off by a large number of steps.  This lack of a clear dividing line made it difficult to compute a pull-in frequency.  

We conducted 50 trials using the system in the loaded and unloaded cases at $490$ Hz and compared the number of failures reported in table \ref{q3b_table}
...
\xxx{incomplete need to talk about how these aren't really all that different unless we redo the section}

Our LabView system was unable to provide sufficient maximum frequencies to cause the system to desynchronize so we again resorted to the function generator to determine the pull-out frequency.  Testing the system at full steps, we used the extra counter input to record the number of pulses sent by the function generator.  It was then quite simple to compare the measured number of steps, computed from the encoder angle, with the recorded number of steps as shown in figure \xxx{FIGURE}.  

\xxx{picture of labview, had the wrong number entered for the computation, change to 1.8 for full steps, remove extra clutter}

When these two numbers diverge, we know that the stepper motor has lost synchronization.  \xxx{Unlike our trials to determine the pull-in frequencies, this proved to be very clear cut and very repeatable}  The pull-out frequency was determined simply by using the function generator and manually ramping the output frequency up until the motor lost synchronism.  The frequencies determined by these tests for the loaded and unloaded cases are shown in table \ref{q3b_table2}.
