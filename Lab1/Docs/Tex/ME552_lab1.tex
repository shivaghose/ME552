...
Experimentally this seems to be the case at a 1Hz input, but it is not the case for the 100 Hz input.  We can notice that at 1Hz the input amplitudes choosen avoid saturating the system, while the 100Hz signal saturates the system such that output can not increase with increasing input voltage \xxx{What voltage do we expect to saturate at given the sillyness with that multiplier Shiva?}
