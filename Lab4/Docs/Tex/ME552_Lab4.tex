...
$\theta$ is represented as $t$. For our normalization, we require $t$ to be positive and we keep track of the original sign of $t$ through the variable $sign$. $tout$ represents the output value of $mod2\pi(\theta)$ . The $floor$ operation essentially finds the quotient of dividing $t$ by $2\pi$ and the the $+ 0.5$ operation allows the angle to be normalized between $+\pi$ on one side and $-\pi$ on the other. This way, we can constrain $\theta$ to stay between $\pm \pi$.
...
\xxx{Still need bode plots! for design process}

\begin{figure}
\begin{center}
\includegraphics[width = 15cm]{ClassicalLimitCycle.png}
\end{center}
\caption{Limit Cycle exerted by the Classical Controller when applied to the non-linear Model}
...
\xxx{still need to compare model performance with reality}
